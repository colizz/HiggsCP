...
int IsTruthHtt (LCCollection* col_MCP) {
    ...
    for (int i=0; i<nMCP; i++) {
        MCParticle* mcpi = dynamic_cast<MCParticle*>(col_MCP->getElementAt(i));
        if ( mcpi->getPDG()==25 ){
            if (mcpi->getDaughters()[0]->getPDG()==25)  continue;
            HDecayTo = abs( mcpi->getDaughters()[0]->getPDG() );
            break;
        }
    }
    return (HDecayTo==15);
...
std::pair<enum TauMode, enum TauMode> GetTruthTauDecayMode (LCCollection* col_MCP) {
    ...
    // find htt event from bottom to top, find where tau+/- appears the last time
        MCParticle* mcpi = dynamic_cast<MCParticle*>(col_MCP->getElementAt(i));
        if (abs(mcpi->getPDG()) == 15 && mcpi->getDaughters().size() > 1) {
            if ( mcpi->getPDG() == 15 && TauM==nullptr )
                TauM = mcpi;
            if ( mcpi->getPDG() == -15 && TauP==nullptr )
                TauP = mcpi;
        }
